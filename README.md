...
```
If the outputs match the values above, your files are complete and correct. If not, you need to check the split files one by one to find out the broken one, and just download it again. You can check them by the following:<br>
...
```
Run the main code for test subset undersampling, run MATLAB script (original) or a Python script (re-implementation):
```
...
2.  Fanwen Wang et al., Towards modality- and sampling-universal learning strategies for accelerating cardiovascular imaging: Summary of the CMRxRecon2024 challenge, ***IEEE Transactions on Medical Imaging***, 45: 1872-1887, 2026.<br>
...
All used public datasets are also available on their websites, including https://github.com/CmrxRecon, https://ocmr.info, and https://www.ukbiobank.ac.uk. For UK Biobank, the imaging data and non-imaging participant characteristics are available to approved researchers via a standard application process at http://www.ukbiobank.ac.uk/register-apply.
